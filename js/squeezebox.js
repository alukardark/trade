...
export const accordion = ($wrapperEl = '.accordion') => {
    ...
    _Squeezebox.prototype = {
        init : function(){
            var self = this;
            this.wrapper = document.querySelectorAll($wrapperEl);
    ...
    if (document.querySelector('.accordion')) {
        new Squeezebox({
            wrapperEl: $wrapperEl,
            headersClass: 'accordion-head',
            foldersClass: 'accordion-cont',
            speed: '.3s',
...
event('.accordion-head', 'click', function (e) {
    e.preventDefault();
    let accordionHead = this;
    Array.prototype.filter.call(accordionHead.parentNode.children, function (child) {
        if (child !== accordionHead && child.classList.contains('accordion-head') && child.classList.contains('open')) {
            child.click();
        }
    });
});
